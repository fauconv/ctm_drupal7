...
function create_site {
  ...
  sed "s|${PATTERN}|${BASE_ROOT_LIST}|1" ${ABS_SITE_PATH}/settings.php > ${ABS_SITE_PATH}/settings2.php
  ...
  do
    ...
    if [ ! "${ALIAS}" = "" ]; then
      ...
      if [ "$FOUND" = "" ]; then
        ...
        sed "s|CTM_MANAGER_TAG|${TEXT}|" .htaccess > .htaccess2
        rm .htaccess
...
function site_deploy {
  ...
  ${ABS_SCRIPTS_PATH}/drush @${ID} ctm_queue
  if [ "${ENV}" = "dev" ]; then
