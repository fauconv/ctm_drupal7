...
printHelp(){
   ...
   echo "$(basename $0) -b <site-ref> <file>     --> revert a database with the dumped file (TODO)"
   echo "$(basename $0) -u                       --> update all installed site"
   echo "$(basename $0) -t <site-ref>            --> create a package for production/staging (TODO)"
   echo "$(basename $0) -z <tgz-file>            --> update a drupal farm with the zip-file provided by -t"
